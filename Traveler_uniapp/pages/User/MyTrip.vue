<template>
	<view>
		...
		<view >
			...
			<!-- <choiselist :data="tripData"></choiselist> -->
		</view>
	...
	export default {
		...
		methods: {
			...
			goToPage(id,name) {
				if (!id) return;
				uni.navigateTo({
					url: '/pages/Eider/TripEider' +'?id='+ encodeURIComponent(id) + '&name=' + encodeURIComponent(name)
				});
			},
			//接收菜单结果
			confirm(e) {
			...
			getSearch(){
				...
				if ( this.searchData !== '') {
					this.$http.get('trip/?page='+this.page + '&belong=my' + '&search=' + this.searchData).then(res=> {
					if (res.data.results.code === 200) {
						console.log(res.data.results.data)
						this.tripData = res.data.results.data
					}
					})
				} else {
					this.$http.get('trip/?page='+this.page + '&belong=my').then(res=> {
					if (res.data.results.code === 200) {
						console.log(res.data.results.data)
						this.tripData = res.data.results.data
					}
		...
		onReachBottom() {
			...
			this.tripData.push(...this.nextData)
			this.nextData = []
