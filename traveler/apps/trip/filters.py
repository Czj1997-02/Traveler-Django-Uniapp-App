    ...
    def filter_show(self, queryset, name, value):
        ...
        if value == 'undefined':
            ...
            if value == 'follow':
                ...
                #获取点赞列表
                praises = TripPraise.objects.filter(deleted__exact='0',created_by = self.request.user.extension)
                praises_ids = []
                for praise in praises:
                    praises_ids.append(praise.trip.id)
                return queryset.filter(deleted__exact='0').filter(id__in = praises_ids)
            
            elif value == 'collect':
                #获取点赞列表
                collects = TripCollect.objects.filter(deleted__exact='0',created_by = self.request.user.extension)
                collects_ids = []
                for collect in collects:
                    collects_ids.append(collect.trip.id)
